 ...
 * events: ev_1ms|ev_on_SpeedUpdate
...
const RTE_cyclicTable_t RTE_cyclicActivationTable_tsk_io[] = {
	{ JOYSTICK_readJoystick_run, 10 },	//Runnable that reads the joystick value x,y every 10ms and updates through API pullport().
	{ ENGINE_setEngine_run, 100 },	//Runnable that copies the speed signal value to the engine signal value and calls the driver pushport().
};
...
const RTE_eventTable_t RTE_eventActivationTable_tsk_io[] = {
    { BRAKELIGHT_setBrakeLight_run, ev_on_SpeedUpdate },  //Runnable that is updated on speed signal value change.
}; 
...
TASK(tsk_io)
{
    ...
    while(1)
    {
        ...
        WaitEvent(ev_1ms|ev_on_SpeedUpdate);
        GetEvent(tsk_io,&ev);
        ClearEvent(ev);
